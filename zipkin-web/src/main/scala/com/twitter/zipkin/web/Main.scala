...
trait ZipkinWebFactory { self: App =>
    ...
    def newWebServer(
        ...
        stats: StatsReceiver = DefaultStatsReceiver.scope("zipkin-web")): Service[Request, Response] = {
        ...
        Seq(
            ...
            ("/durations", addLayout andThen handleDurations()),
            ...
            ("/api/pin/:id/:state", handleTogglePin(queryClient, webPinTtl()))).foldLeft(new HttpMuxer) {
                case (m, (p, handler)) =>
